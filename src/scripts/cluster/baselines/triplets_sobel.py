...
from src.utils.cluster.baselines.triplets import make_triplets_data, \
    triplets_eval, triplets_loss
from src.utils.cluster.transforms import sobel_process
from src.utils.cluster.general import config_to_str, get_opt, update_lr
...
for e_i in range(next_epoch, config.num_epochs):
    ...
    for tup in itertools.izip(*iterators):
        ...
        # no sobel yet
        imgs_orig = sobel_process(tup[0][0], config.include_rgb)
        imgs_pos = sobel_process(tup[1][0], config.include_rgb)
        imgs_neg = sobel_process(tup[2][0], config.include_rgb)
