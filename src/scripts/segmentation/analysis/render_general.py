...
parser.add_argument("--imgs_dataloaders", type=str,
                    nargs="+", default=["test"])
        ...
        for net_name in net_names:
            ...
            for b_i, batch in enumerate(imgs_dataloader):
                ...
                orig_imgs, flat_targets, mask = \
                    orig_imgs, flat_targets.numpy(), mask.numpy().astype(np.bool)
                ...
                # not in colourmaps, hence will be black
                flat_targets[masked] = -1
                ...
                if "Coco" in config.dataset:
                    ...
                    # ignore others
                    orig_imgs_swapped[:, 2, :, :] = orig_imgs[:, 0, :, :]
                ...
                print("... rendered batch %d, next_img_ind %d " %
                      (b_i, next_img_ind))
                sys.stdout.flush()
