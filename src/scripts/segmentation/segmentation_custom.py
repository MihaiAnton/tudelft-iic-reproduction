...
def train():
    ...
    print("Train 2 ", datetime.now())
    net = archs.__dict__[config.arch](config)
    if config.restart:
        print("Train 2.5 ", datetime.now())
        dict = torch.load(os.path.join(config.out_dir, dict_name),
                          map_location=lambda storage, loc: storage)
        net.load_state_dict(dict)
    # net
    print("Train 3 ", datetime.now())
    net = torch.nn.DataParallel(net)
    net.train()

    print("Train 4 ", datetime.now())
    optimiser = get_opt(config.opt)(net.module.parameters(), lr=config.lr)
    # if config.restart:
    #     optimiser.load_state_dict(dict["optimiser"])

    print("Train 5 ", datetime.now())
    heads = ["A", "B"]
    ...
    print("Train", next_epoch, config.num_epochs)
    # Train
    # ------------------------------------------------------------------------
    ...
    #                                    config.input_sz, config.input_sz).to(
    #                 torch.float32)
    #             all_affine2_to_1 = torch.zeros(config.batch_sz, 2, 3).to(  # TODO ask about this
    #                 torch.float32)
    #             all_mask_img1 = torch.zeros(config.batch_sz, config.input_sz,
    #                                         config.input_sz).to(torch.float32)  # TODO ask about this
