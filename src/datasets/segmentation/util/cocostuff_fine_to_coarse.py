...
def generate_fine_to_coarse(out_path):
    out_path = '/Users/mihai/Documents/School/TUDelft/DeepLearning/IIC/src/datasets/segmentation/util/out/fine_to_coarse_dict.pickle'
    print("generating fine to coarse files in %s ..." % out_path)
    # -1 (unlabelled) is not in dict, not used (see _CocoStuff._fine_to_coarse)

    print(os.getcwd())

    with open("./src/datasets/segmentation/util/cocostuff_fine_raw.txt") as f:
        l = [tuple(pair.rstrip().split('\t')) for pair in f]
        l = [(int(ind), name) for ind, name in l]

    with open("./src/datasets/segmentation/util/cocostuff_hierarchy.y") as f:
        d = yaml.load(f)
    ...
    with open(out_path + ".txt", "w") as out_f:
        ...
        for k in fine_name_to_coarse_name.keys():
            v = fine_name_to_coarse_name[k]
            out_f.write("%s\t%s" % (k, v))

        print("fine_index_to_coarse_index:")
        for k in fine_index_to_coarse_index.keys():
            v = fine_index_to_coarse_index[k]
            out_f.write("%d\t%d" % (k, v))
